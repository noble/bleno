...
Bleno.prototype.startAdvertising = function(name, serviceUuids, callback) {
  var advertise = function(state) {

    if (state !== 'poweredOn') {
      var error = new Error('Could not start advertising, state is ' + state + ' (not poweredOn)');
  ...
  if (!this.initialized) {
    this._bindings.init();
    this.initialized = true;
    this.once('stateChange', advertise.bind(this));
  }else{
    advertise.call(this, this._state);
  }
};
...
Bleno.prototype.startAdvertisingWithEIRData = function(advertisementData, scanData, callback) {
  ...
  var advertise = function(state) {
    ...
    if (state !== 'poweredOn') {
      var error = new Error('Could not advertising scanning, state is ' + state + ' (not poweredOn)');
