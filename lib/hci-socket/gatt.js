...
function createATTMessage(op, mtu, valueHandle, data){
...
function createServiceHandles(service, start){
  var handle = start;
  var handles = [];
  ...
  handles[0].endHandle = handle;

  return handles;
}

Gatt.prototype.addService = function AddGATTService(service){
  this._services.push(service);
  var handles = createServiceHandles(service, this._handles.length - 1);
  for(var i = 0, ii = handles.length; i < ii; i++){
      var handle = handles[i].startHandle || handles[i].handle;
      this._handles[handle] = handles[i];
  }

  // Indicate that services changed.
var serviceHandleObject = handles[0];

  var serviceChangedValue = new Buffer(4);
  serviceChangedValue.writeUInt16LE(parseInt(serviceHandleObject.startHandle, 16), 0);
  serviceChangedValue.writeUInt16LE(parseInt(serviceHandleObject.endHandle, 16), 2);
  this._handles[this._serviceChangedValueHandle].value = serviceChangedValue;

  var indicateMessage = createATTMessage(ATT_OP_HANDLE_IND, this._mtu, this._serviceChangedValueHandle, serviceChangedValue);
  this._lastIndicatedAttribute = this._handles[this._serviceChangedValueHandle - 1].attribute;

  debug('addService: indicate message: ' + indicateMessage.toString('hex'));
...
Gatt.prototype.removeAllServices = function RemoveAllGATTServices(){
  ...
  var indicateMessage = createATTMessage(ATT_OP_HANDLE_IND, this._mtu, this._serviceChangedValueHandle,
    serviceChangedValue);
};
...
Gatt.prototype.removeService = function RemoveGattService(service){
  ...
  var startHandle = this._handles[handle].startHandle;
  var endHandle = this._handles[handle].endHandle;
  ...
  debug('removeService: indicate message: ' + indicateMessage.toString('hex'));
  this.send(indicateMessage);
};
...
Gatt.prototype.setServices = function SetGATTServices(services) {
  ...
  this._services[0].characteristics[0].value = new Buffer(deviceName);
  services = services || [];
  ...
  for (var i = 0, ii = this._services.length; i < ii; i++) {
    var service = this._services[i];

    // A bit hacky, but keeps indexing proper so access can be this._handles[index], concat will leave empty properties
    var serviceHandles = createServiceHandles(service, this._handles.length - 1);
    for(var j = 0, jj = serviceHandles.length; j < jj; j++){
        var handle = serviceHandles[j].startHandle || serviceHandles[j].handle;
        this._handles[handle] = serviceHandles[j];

        if (this._handles[handle].uuid === '2a05'){
          this._serviceChangedValueHandle = handle;
        }
    }
  }
          ...
          if (value & 0x0003) {
            var updateValueCallback = (function(valueHandle, attribute) {
              return function(data) {
                ...
                if (useNotify) {
                  var notifyMessage = createATTMessage(ATT_OP_HANDLE_NOTIFY, this._mtu, valueHandle, data);
                  ...
                  var indicateMessage = createATTMessage(ATT_OP_HANDLE_IND, this._mtu, valueHandle, data);
