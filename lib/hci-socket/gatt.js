...
Gatt.prototype.addService = function AddGATTService(service, indicate){
  ...
  for (var j = 0; j < service.characteristics.length; j++) {
    ...
    for (var k = 0, kk = descriptors.length; k < kk; k++) {
  ...
  if(!handles.length){
      attHandle = 1;
  } else if(parseInt((serviceHandles.length + handles.length), 16) >= 0xFFFF){
      ...
      for(var i = 0, ii = handles.length; i < ii; i++){
        ...
        if(!handle.uuid){
          consecutiveCount++;
        } else if(handle.uuid && consecutiveCount) {
      ...
      var attributeCount = serviceHandles.length;
      if(groupedOpenings[attributeCount]){
          attHandle = groupedOpenings[attributeCount].start;
      } else {
          for(var openingCount in groupedOpenings){
              if(parseInt(openingCount, 10) >= attributeCount){
                  attHandle = groupedOpenings[openingCount].start;
              }
          }
      }
      ...
      /**
       * This is put in place so that Linux handle management behaves how OS X handle management does.
       * Quick Primer: On OS X, if the last service towards the end of our attributes is taken out, os x will
       * use it's handles if another service is added after. Therefore, on linux we need the service that's
      ...
      attHandle = handles[lastServiceIndex].endHandle + 1;
  }

  var startHandle = attHandle;
  var endHandle = null;
  for(var l = 0, ll = serviceHandles.length; l < ll; l++){
    var serviceHandle = serviceHandles[l];

    if (serviceHandle.type === 'service'){
      serviceHandle.startHandle = attHandle;

      endHandle = attHandle + serviceHandles.length -1;
      serviceHandle.endHandle = endHandle;
    } else if(serviceHandle.type === 'characteristic'){
        serviceHandle.startHandle = attHandle;
        serviceHandle.valueHandle = (attHandle + 1);

        if (serviceHandle.uuid === '2a05'){
          this._serviceChangedValueHandle = serviceHandle.valueHandle;
        }
    } else if(serviceHandle.type === 'characteristicValue' || serviceHandle.type === 'descriptor'){
        serviceHandle.handle = attHandle;
    }
    handles[attHandle] = serviceHandle;
    attHandle++;
  }

  if(indicate){
      this.serviceChanged(startHandle, endHandle);
  }

  // The instanced service should store the startHandle for removal later...if necessary.
  service.handle = startHandle;

  debug('addService: added handles ' + startHandle + '-' + endHandle);
};

Gatt.prototype.removeAllServices = function RemoveAllGATTServices(){
  var handles = this._handles;
  for (var i = 0; i < handles.length; i++) {
    var handle = handles[i];
    if(handle.uuid === '1800' || handle.uuid === '1801' || handle.uuid === '2a00' || handle.uuid === '2a01' ||
      handle.uuid === '2a05'){
      continue;
    } else {
        handle = {};
  ...
  // Make the central rescan all services Reference Bluetooth Spec Vol.3.G 2.5.2
  this.serviceChanged(0x0001, 0xFFFF);
  debug('removedAllServices');
};

Gatt.prototype.removeService = function RemoveGattService(service){
  var handle = service.handle;
  var handles = this._handles;

  var startHandle = handles[handle].startHandle;
  var endHandle = handles[handle].endHandle;
  for(var handle = startHandle; handle <= endHandle; handle++){
      // Don't slice, indexing will be corrupted.
      handles[handle] = {};
  }
  this.serviceChanged(startHandle, endHandle);

  debug('removeService - removed handles: ' + startHandle + '-' + endHandle);
};

/**
 * Set services for the GATT server for discovery. Do not call this function if you are connected to a client, it can have
 * negative consequences if the services are in a different order from the time it was initially called.
 * @param   {Array<Service>} services The services that will be available for Primary Service Discovery Ref. Vol 3 GATT 4.13
 * @returns {undefined}          No payload data provided for return.
 */
Gatt.prototype.setServices = function SetGATTServices(services) {
  ...
  services = baseServices.concat(services);
  this._handles = [];
...
Gatt.prototype.serviceChanged = function ServiceChanged(startHandle, endHandle){
    if (!this._serviceChangedSubscribed){
        return;
    }
    var serviceChangedValue = new Buffer(4);
    serviceChangedValue.writeUInt16LE(parseInt(startHandle, 16), 0);
    serviceChangedValue.writeUInt16LE(parseInt(endHandle, 16), 2);
    this._handles[this._serviceChangedValueHandle].value = serviceChangedValue;

    this.sendATTMessage(ATT_OP_HANDLE_IND,  this._serviceChangedValueHandle, serviceChangedValue);
    this._lastIndicatedAttribute = this._handles[this._serviceChangedValueHandle - 1].attribute;
};
...
Gatt.prototype.sendATTMessage = function SendATTMessage(op, valueHandle, data){
        var mtu = this._mtu;
        var dataLength = Math.min(data.length, mtu - 3);
        var message = new Buffer(3 + dataLength);

        message.writeUInt8(op, 0);
        message.writeUInt16LE(valueHandle, 1);
...
Gatt.prototype.handleWriteRequestOrCommand = function handleWriteRequestOrCommand(request) {
  var response = null;
    ...
    if (handleProperties && (withoutResponse ? (handleProperties & 0x04) : (handleProperties & 0x08))) {
      ...
      if (handleSecure & (withoutResponse ? 0x04 : 0x08) && !this._aclStream.encrypted) {
        ...
        if (data.length !== 2) {
          ...
          if (value & 0x0003) {
            // No need to give callback for service changed characteristic
            if(handleAttribute.uuid !== '2a05'){
                var updateValueCallback = (function(valueHandle, attribute) {
                  return function(data) {
                    ...
                    if (useNotify) {
                      this.sendATTMessage(ATT_OP_HANDLE_NOTIFY, valueHandle, data);

                      attribute.emit('notify');
                    } else if (useIndicate) {
                      this.sendATTMessage(ATT_OP_HANDLE_IND, valueHandle, data);
                ...
                this._serviceChangedSubscribed = true;
            }
          } else {
            if (handleAttribute.uuid !== '2a05'){
                handleAttribute.emit('unsubscribe');
            } else {
                this._serviceChangedSubscribed = false;
            }
          }
          result = ATT_ECODE_SUCCESS;
        }
