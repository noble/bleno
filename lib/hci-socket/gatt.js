...
Gatt.prototype.addService = function AddGATTService(service, indicate){
  ...
  for(var l = 0, ll = serviceHandles.length; l < ll; l++){
    ...
    if (serviceHandle.type === 'service'){
      ...
      serviceHandle.endHandle = (attHandle + serviceHandles.length);
    ...
    } else if(serviceHandle.type === 'characteristicValue' || characteristic.type === 'descriptor'){
  ...
  var endHandle = startHandle + serviceHandles.length;
  if(indicate){
...
Gatt.prototype.sendATTMessage = function SendATTMessage(op, valueHandle, data){
        ...
        message.writeUInt8(parseInt(op, 16), 0);
        message.writeUInt16LE(parseInt(valueHandle, 16), 1);
        ...
        debug('sendATTMessage: ' + msg + '-' + message.toString('hex'));
}
