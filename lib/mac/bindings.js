  ...
  if (services.length) {
    for (var i = 0; i < services.length; i++) {
      ...
      var arg = {
        ...
        kCBMsgArgType: (service.primary ? 1 : 0),
        kCBMsgArgUUID: new Buffer(service.uuid, 'hex')
      };

      if (service.primary && service.included && service.included.length) {
        for (var includedIndex = 0; includedIndex < service.included.length; includedIndex++) {
          var includedServiceId = this._attributes.indexOf(service.included[includedIndex]);
          if (includedServiceId !== -1) {
            arg.kCBMsgArgAttributeIDs.push(includedServiceId);
          }
        }
      }
