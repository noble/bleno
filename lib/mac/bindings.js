...
blenoBindings.addService = function(service) {
  var attributeId = this._attributes.length;
...
blenoBindings.removeService = function(service) {
  var attributeId = service.attributeId;
  var attribute = this._attributes[attributeId];

  for (var i = 0; i < attribute.characteristics.length; i++) {
    var characteristic = attribute.characteristics[i];

    this._attributes[characteristic.attributeId] = {};
  }

  this._attributes[attributeId] = {};
...
blenoBindings.removeAllServices = function() {
  this._attributes = [];
...
blenoBindings.setServices = function SetGATTServices(services) {
  ...
  if (services.length) {
    ...
    this._lastServiceAttributeId = service.attributeId;
  } else {
...
blenoBindings.on('kCBMsgId18', function(args) {
  ...
  if (attributeId === this._lastServiceAttributeId) {
    ...
    this._lastServiceAttributeId = null;
  }
});
