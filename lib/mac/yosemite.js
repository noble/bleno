...
blenoBindings.startAdvertising = function(name, serviceUuids) {
  ...
  if (serviceUuids && serviceUuids.length) {
    for(var i = 0; i < serviceUuids.length; i++) {
      advertisement.kCBAdvDataServiceUUIDs[i] = Buffer.from(serviceUuids[i], 'hex');
    }
  }
...
blenoBindings.startAdvertisingIBeacon = function(data) {
  ...
  if (osRelease >= 14) {
    args.kCBAdvDataAppleMfgData = Buffer.concat([
      Buffer.from([data.length + 5, 0xff, 0x4c, 0x00, 0x02, data.length]),
      data
    ]);
  } else {
  ...
  if (services.length) {
    for (var i = 0; i < services.length; i++) {
      ...
      var arg = {
        ...
        kCBMsgArgUUID: Buffer.from(service.uuid, 'hex')
      };
      ...
      for (var j = 0; j < service.characteristics.length; j++) {
        ...
        var characteristicArg = {
          ...
          kCBMsgArgUUID: Buffer.from(characteristic.uuid, 'hex')
        };
        ...
        for (var k = 0; k < characteristic.descriptors.length; k++) {
          ...
          characteristicArg.kCBMsgArgDescriptors.push({
            ...
            kCBMsgArgUUID: Buffer.from(descriptor.uuid, 'hex')
          });
        }
...
blenoBindings.on('kCBMsgId53', function(args) {
  ...
  this._deviceUUID = Buffer.from(deviceUUID, 'hex');
