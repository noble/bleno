...
Hci.prototype.processLeLtkRequest = function(status, data) {
  var handle = data.readUInt16LE(0);
  var random = data.slice(2, 10);
  var diversifier = data.slice(10);

  debug('\t\t\thandle = ' + handle);
  debug('\t\t\trandom = ' + random.toString('hex'));
  debug('\t\t\tdiversifier = ' + diversifier.toString('hex'));

  this.emit('leLtkRequest', handle, random, diversifier);
};
