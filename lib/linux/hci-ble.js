...
HciBle.prototype.startAdvertising = function(advertisementDict) {
  debug('startAdvertising: dict = ' + JSON.stringify(advertisementDict));
  
  name = advertisementDict['name'] || "";
  serviceUuids = advertisementDict['serviceUuids'] || [];
  manufacturerSpecifcData = advertisementDict['manufacturerSpecificData'] || "";
  ...
  if(manufacturerSpecifcData.length) {
    advertisementDataLength += 2 + manufacturerSpecifcData.length;
  }
  ...
  if(manufacturerSpecifcData.length) {
    advertisementData[i++] = 1 + manufacturerSpecifcData.length;
    advertisementData[i++] = 0xFF;
    for(j = 0; j < manufacturerSpecifcData.length; j++) {
        advertisementData[i++] = manufacturerSpecifcData[j];
    }
  }
