...
L2capBle.prototype.handleReadOrReadBlobRequest = function(request) {
  ...
  if (handle) {
    ...
    if (handleType === 'service' || handleType === 'includedService') {
      ...
      if (this._handles[valueHandle - 1].properties & 0x02) {
        data = this._handles[valueHandle].value;
      } else {
        errorCode = ATT_ECODE_READ_NOT_PERM; // non-readable
      }
    }
  }
  ...
  if (errorCode) {
    response = this.errorResponse(requestType, valueHandle, errorCode);
  } else if (data) {
    var dataLength = Math.min(data.length, this._mtu - 1);
    response = new Buffer(1 + dataLength);

    response[0] = (requestType === ATT_OP_READ_BLOB_REQ) ? ATT_OP_READ_BLOB_RESP : ATT_OP_READ_RESP;
    for (i = 0; i < dataLength; i++) {
      response[1 + i] = data[i];
    }
  } else {
