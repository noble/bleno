...
L2capBle.prototype.setServices = function(services) {
  ...
  for (i = 0; i < allServices.length; i++) {
    ...
    if (service.absoluteHandle === void 0) {
      if (service.relativeHandle === void 0) {
        handle++;
      } else {
        debug('Got service relativeHandle! Hex handle=', service.relativeHandle, ' Handle=', handle);
        handle += parseInt(service.relativeHandle, 16);
      }
    } else {
      debug('Got service absoluteHandle! Hex handle=', service.absoluteHandle, ' Handle=', handle);
      handle = parseInt(service.absoluteHandle, 16);
    }
    var serviceHandle = handle;

    this._handles[serviceHandle] = {
      type: service.isSecondary ? 'secondaryService' : 'service',
      uuid: service.uuid,
      attribute: service,
    ...
    //included services
    for (j = 0; j < service.included.length; j++) {
      handle++;
      this._handles[handle] = {
        type: 'includedService',
        ID: service.included[j]
      }
    }
    ...
    for (j = 0; j < service.characteristics.length; j++) {
      ...
      if (characteristic.relativeHandle === void 0) {
        handle++;
      } else {
        debug('Got characteristic relativeHandle! Hex handle=', characteristic.relativeHandle, ' Handle=', handle);
        handle += parseInt(characteristic.relativeHandle, 16);
      }

      var characteristicHandle = handle;

      if (characteristic.relativeValueHandle === void 0) {
        handle++;
      } else {
        debug('Got characteristic relativeValueHandle! Hex handle=', characteristic.relativeValueHandle, ' Handle=', handle);
        handle += parseInt(characteristic.relativeValueHandle, 16);
      }
      ...
      this._handles[characteristicValueHandle] = {
        ...
        characteristicsHandle: characteristicHandle
      };
      ...
      for (var k = 0; k < characteristic.descriptors.length; k++) {
        ...
        if (descriptor.relativeHandle !== void 0){
            debug('Got descriptor relativeHandle! Hex handle=', descriptor.relativeHandle, ' Handle=', handle);
            handle += parseInt(descriptor.relativeHandle, 16);
        }else{
            handle++;
        }
  ...
  //fill includes services
  for (i = 0; i < this._handles.length; i++) {
    handle = this._handles[i];

    if (handle !== void 0 && handle.type === 'includedService'){
      debug('Found included !', handle);
      var found = false;
      for (j = 0; j < this._handles.length; j++){
        var currentHandle = this._handles[j];

        if (i !==j &&
          currentHandle !== void 0 &&
          currentHandle.ID !== void 0 &&
          handle.ID !== void 0 &&
          currentHandle.ID === handle.ID){

          handle.uuid = currentHandle.uuid;
          handle.startHandle = currentHandle.startHandle;
          handle.endHandle = currentHandle.endHandle;
          attribute= currentHandle;
          debug('Found !', handle, currentHandle);
        }
      }
    }
  }
...
L2capBle.prototype.handleFindInfoRequest = function(request) {
  ...
  for (i = startHandle; i <= endHandle; i++) {
    ...
    switch(handle.type){
      case 'service': uuid = '2800'; break;
      case 'secondaryService': uuid = '2801'; break;
      case 'includedService': uuid = '2802'; break;
      case 'characteristic': uuid = '2803'; break;
      case 'characteristicValue': uuid = this._handles[i].characteristicsUuid;; break;
      case 'descriptor': uuid = handle.uuid; break;
    }
...
L2capBle.prototype.handleFindByTypeRequest = function(request) {
  ...
  debug('Find By Type: startHandle = 0x' + startHandle.toString(16) +
    ', endHandle = 0x' + endHandle.toString(16) +
  ...
  for (var i = startHandle; i <= endHandle; i++) {
    ...
    if (('2800' === uuid && handle.type === 'service' && handle.uuid === value) ||
       ('2801' === uuid && handle.type === 'secondaryService' && handle.uuid === value)) {
      handles.push({
...
L2capBle.prototype.handleReadByGroupRequest = function(request) {
  ...
  if ('2800' === uuid || '2801' === uuid || '2802' === uuid) {
    var services = [];
    var i;
    var type = '';
    switch (uuid){
      case '2800': type = 'service'; break;
      case '2801': type = 'secondaryService'; break;
      case '2802': type = 'includedService'; break;
    }
...
L2capBle.prototype.handleReadByTypeRequest = function(request) {
  ...
  if ('2803' === uuid) {
    ...
    debug('Read by type 2802');
    var valueHandle = null;
    var foundHandleIdx = -1;
    ...
    for (i = startHandle; i <= endHandle; i++) {
      ...
      if (handle.type === 'includedService') {
        valueHandle = handle;
        foundHandleIdx = i;
        break;
      }
    ...
    if (valueHandle) {
      var shortUUID = (valueHandle.uuid.length === 4);
      var dataLength =  6;
      if (shortUUID) {
        dataLength+=2;
      }
      response = new Buffer(2 + dataLength);
      response[0] = ATT_OP_READ_BY_TYPE_RESP;
      response[1] = dataLength;

      response.writeUInt16LE(foundHandleIdx, 2);
      response.writeUInt16LE(valueHandle.startHandle, 4);
      response.writeUInt16LE(valueHandle.endHandle, 6);

      if (shortUUID){
        data = new Buffer(handle.uuid.match(/.{1,2}/g).reverse().join(''), 'hex');
        data.copy(response,8);
      }
    } else {
      response = this.errorResponse(ATT_OP_READ_BY_TYPE_REQ, startHandle, ATT_ECODE_ATTR_NOT_FOUND);
    }
  } else {
    debug('Read by type - other');
    var valueHandle = null;
    var secure = false;
...
L2capBle.prototype.handleReadOrReadBlobRequest = function(request) {
  ...
  if (handle) {
    ...
    if (handleType === 'service' || handleType === 'secondaryService'|| handleType === 'includedService') {
      ...
      if (handleType === 'characteristicValue') {
        var characteristicsHandle = this._handles[valueHandle].characteristicsHandle;
        handleProperties = this._handles[characteristicsHandle].properties;
        handleSecure = this._handles[characteristicsHandle].secure;
        handleAttribute = this._handles[characteristicsHandle].attribute;
      }
...
L2capBle.prototype.handleWriteRequestOrCommand = function(request) {
  ...
  if (handle) {
    if (handle.type === 'characteristicValue') {
      handle = this._handles[this._handles[valueHandle].characteristicsHandle];
    }
    ...
    if (handleProperties && (withoutResponse ? (handleProperties & 0x04) : (handleProperties & 0x08))) {
      ...
      if (handleSecure & (withoutResponse ? 0x04 : 0x08) && (this._security !== 'medium' && this._security !== 'high')) {
        ...
        if (handle.type === 'descriptor' || handle.uuid === '2902') {
        var result = null;
