  ...
  if ('2800' === uuid || '2802' === uuid) {
    ...
    if (services.length) {
      ...
      for (var i = 0; i < numServices; i++) {
        ...
        var uuid = new Buffer(service.uuid.match(/.{1,2}/g).reverse().join(''), 'hex');
        for (var j = 0; j < uuid.length; j++) {
          response[2 + i * lengthPerService + 4 + j] = uuid[j];
        }
      }
    } else {
...
L2capBle.prototype.handleReadByTypeRequest = function(request) {
  ...
  if ('2803' === uuid) {
    ...
    if (characteristics.length) {
      ...
      for (var i = 0; i < numCharacteristics; i++) {
        ...
        var uuid = new Buffer(characteristic.uuid.match(/.{1,2}/g).reverse().join(''), 'hex');
        for (var j = 0; j < uuid.length; j++) {
          response[2 + i * lengthPerCharacteristic + 5 + j] = uuid[j];
        }
      }
    } else {
