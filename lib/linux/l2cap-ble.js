...
L2capBle.prototype.handlePrepareWriteRequest = function(request) {
  var response = null;
    ...
    if (handleProperties && (handleProperties & 0x08)) {
      ...
      var callback = (function(request) {
        return function(result) {
          ...
          if (ATT_ECODE_SUCCESS === result) {
            callbackResponse = new Buffer(request.length);
            request.copy(callbackResponse);
            callbackResponse[0] = ATT_OP_PREP_WRITE_RESP;
          } else {
            callbackResponse = this.errorResponse(requestType, valueHandle, result);
          }
          debug('prepare write response: ' + callbackResponse.toString('hex'));
      ...
      if (handleSecure & 0x08 && (this._security !== 'medium' && this._security !== 'high')) {
        ...
        this._prepWriteValueHandle = valueHandle;
        handle.attribute.emit('prepareWriteRequest', data, offset, callback);
      }
    } else {
      response = this.errorResponse(requestType, valueHandle, ATT_ECODE_WRITE_NOT_PERM);
    }
...
L2capBle.prototype.handleExecuteWriteRequest = function(request) {
  var response = null;

  var requestType = request[0];
  var flag = request[1];
  var valueHandle = this._prepWriteValueHandle;
  ...
  if (handle) {
    ...
    var callback = (function(flag) {
      return function(result) {
        ...
        debug('exec write response: ' + callbackResponse.toString('hex'));

        this.send(callbackResponse);
      }.bind(this);
    }.bind(this))(flag);

    handle.attribute.emit('executeWriteRequest', flag, callback);
